document.getElementById('footer-wrapper').innerHTML =
'<footer id="footer" class="container">' +
'	<div class="row">' +
'		<div class="3u 12u(mobile)">' +
//Links
'			<section>' +
'				<h2>For Professionals</h2>' +
'				<ul class="divided">' +
'					<li><a href="http://www.ericjearley.com/about/virtualCV.html">Curriculum Vitae</a></li>' +
'					<li><a href="http://www.ericjearley.com/publications.html">Publications</a></li>' +
'					<li><a href="http://www.ericjearley.com/outreach.html">Outreach and Teaching</a></li>' +
'				</ul>' +
'			</section>' +
'		</div>' +
'		<div class="3u 12u(mobile)">'+
//Links
'			<section>' +
'				<h2>For Unprofessionals</h2>' +
'				<ul class="divided">' +
'					<li><a href="http://www.ericjearley.com/about.html">About Me</a></li>' +
'					<li><a href="http://www.ericjearley.com/sciencecommunication.html">Science Communication</a></li>' +
'					<li><a href="#">Blog</a></li>' +
'				</ul>' +
'			</section>' +
'		</div>' +
'		<div class="6u 12u(mobile)">' +
//Contact
'			<section>' +
'				<h2>Get in touch</h2>' +
'				<div class="row">' +
'					<div class="4u 12u(mobile)">' +
'						<table class="footer" style="width: 100px">' +
'							<tr>' +
'								<td><i id="twittericon" class="icon fa-twitter-square"></i></td>' +
'								<td><a href="https://twitter.com/EricJEarley">Twitter</a></td>' +
'							</tr>' +
'							<tr>' +
'								<td><i class="icon fa-linkedin-square"></i></td>' +
'								<td><a href="https://www.linkedin.com/in/ericearley">LinkedIn</a></td>' +
'							</tr>' +
'							<tr>' +
'								<td><i class="icon fa-youtube-square"></i></td>' +
'								<td><a href="https://www.youtube.com/channel/UCiwT2dGNVAU-8R7c-UAEVVw">YouTube</a></td>' +

'							</tr>' +
'							<tr>' +
'								<td><i class="icon fa-envelope"></i></td>' +
'								<td><a href="mailto:earley.eric@gmail.com">Email</a></td>' +
'							</tr>' +
'						</table>' +
'					</div>' +
//Address
'					<div class="8u 12u(mobile)">' +
'						<table class="footer" style="width: 350px">' +
'							<tr>' +
'								<td><b>Address</b></td>' +
'								<td>Rehabilitation Institute of Chicago<br />' +
'								345 E. Superior St., Rm. 1431<br />' +
'								Chicago, IL 60611<br />' +
'								USA</td>' +
'							</tr>' +
'							<tr>' +
'								<td><b>Phone<b></td>' +
'								<td>(312) 238-2079</td>' +
'						</table>' +
'					</div>' +
'				</div>' +
'			</section>' +
'		</div>' +
'	</div>' +
//Copyright
'	<div class="row">' +
'		<div class="12u">' +
'			<div id="copyright">' +
'				<ul class="menu">' +
'					<li>&copy Eric J. Earley. All rights reserved</li><li>Template: <a href="https://html5up.net/zerofour">ZeroFour</a> by <a href="https://twitter.com/ajlkn">AJ</a></li><li>Source: <a href="https://html5up.net">HTML5 UP</a></li>' +
'				</ul>' +
'			</div>' +
'		</div>' +
'	</div>' +
'</footer>';